...
const ordersRoutes = async (req, res) => {
    const [product, id] = parseUrl(req.url);

    if (product === PRODUCTS.ORDERS) {
        try {
            ...
            switch (method) {
                case METHODS.GET:
                    ...
                    break;
                case METHODS.POST:
                    handlers.handlePost(req, res, data, filePath, PRODUCTS.ORDERS, id);
                    break;
                case METHODS.PUT:
                    handlers.hanldePut(req, res, id, data, filePath, PRODUCTS.ORDERS);
                    break;
                case METHODS.PATCH:
                    handlers.handlePatch(req, res, id, data, filePath, PRODUCTS.ORDERS);
                    break;
                case METHODS.DELETE:
